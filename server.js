...
const PANEL_PASSWORD = process.env.PANEL_PASSWORD || "mister4419";
const PANEL_SESSION_SECRET = process.env.PANEL_SESSION_SECRET || process.env.PANEL_PASSWORD || "trevizio-session-local";
const painelSessoes = new Map();
...
function parseCookies(req) {
  ...
  return Object.fromEntries(header.split(";").map((parte) => {
    ...
    const chave = parte.slice(0, index).trim();
    const valor = decodeURIComponent(parte.slice(index + 1).trim());
    return [chave, valor];
  }).filter(Boolean));
}
...
function htmlLogin(mensagem = "") {
  return `<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1.0">
  <title>TreVizio Bot Painel - Login</title>
  <style>
    *{box-sizing:border-box}
    body{font-family:Arial,sans-serif;background:linear-gradient(135deg,#07111f,#0f172a 55%,#111827);color:#e5e7eb;margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
    .login-card{width:100%;max-width:420px;background:#111827;border:1px solid #334155;border-radius:18px;padding:28px;box-shadow:0 20px 70px rgba(0,0,0,.35)}
    .brand{display:flex;align-items:center;gap:12px;margin-bottom:20px}
    .logo{width:48px;height:48px;border-radius:14px;background:linear-gradient(135deg,#22c55e,#38bdf8);display:flex;align-items:center;justify-content:center;color:#03131f;font-size:22px;font-weight:900}
    h1{font-size:24px;margin:0}.small{color:#94a3b8;font-size:14px;margin-top:4px}
    label{display:block;margin-top:18px;margin-bottom:8px;color:#cbd5e1;font-weight:bold}
    input{width:100%;padding:13px;border-radius:12px;border:1px solid #475569;background:#0b1220;color:#fff;font-size:15px}
    button{width:100%;margin-top:18px;background:#22c55e;color:#052e16;border:0;padding:13px;border-radius:12px;font-weight:bold;cursor:pointer;font-size:15px}
    .erro{background:#7f1d1d;color:#fecaca;padding:10px;border-radius:10px;margin-top:14px;font-size:14px}
  </style>
</head>
<body>
  <form class="login-card" method="POST" action="/api/login">
    <div class="brand">
      ...
      <div>
        ...
        <div class="small">Acesso protegido ao painel online</div>
      </div>
    </div>
    <label for="senha">Senha do painel</label>
    <input id="senha" name="senha" type="password" placeholder="Digite a senha" autocomplete="current-password" autofocus>
    <button type="submit">Entrar no painel</button>
    ${mensagem ? `<div class="erro">${mensagem}</div>` : ""}
  </form>
</body>
</html>`;
}
...
function estaAutenticado(req) {
  ...
  const token = cookies.trevizio_panel_session;
  const sessao = token ? painelSessoes.get(token) : null;
  if (!sessao) return false;
  if (Date.now() - sessao.criadoEm > 1000 * 60 * 60 * 24 * 7) {
    painelSessoes.delete(token);
    return false;
  }
  return true;
}
...
function exigirLogin(req, res, next) {
  ...
  if (liberados.includes(caminho)) return next();
  if (estaAutenticado(req)) return next();
  if (caminho.startsWith("/api/")) {
    return res.status(401).json({ ok: false, error: "Acesso não autorizado. Faça login no painel." });
  }
...
app.get("/login", (req, res) => {
  ...
  res.send(htmlLogin(req.query.erro ? "Senha incorreta." : ""));
});
...
app.post("/api/login", express.urlencoded({ extended: false }), (req, res) => {
  ...
  painelSessoes.set(token, { criadoEm: Date.now() });
  const secure = cookieSeguro(req) ? "; Secure" : "";
  res.setHeader("Set-Cookie", `trevizio_panel_session=${encodeURIComponent(token)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${60 * 60 * 24 * 7}${secure}`);
  res.redirect("/");
});

app.get("/api/auth/status", (req, res) => {
  res.json({ ok: true, autenticado: estaAutenticado(req) });
});

app.post("/api/logout", (req, res) => {
  const cookies = parseCookies(req);
  const token = cookies.trevizio_panel_session;
  if (token) painelSessoes.delete(token);
  res.setHeader("Set-Cookie", "trevizio_panel_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0");
  res.json({ ok: true });
});
...
let desconexaoManualEmAndamento = false;
...
function lerMapeamentoLidPorNumero(numero = "") {
  try {
    ...
    if (!n) return null;
    const arquivo = path.join(AUTH_ROOT, `lid-mapping-${n}.json`);
...
function lerNumeroPorLid(lid = "") {
  try {
    ...
    if (!n) return null;
    const arquivo = path.join(AUTH_ROOT, `lid-mapping-${n}_reverse.json`);
    if (!fs.existsSync(arquivo)) return null;
    return normalizarNumero(JSON.parse(fs.readFileSync(arquivo, "utf-8")));
  } catch {
    return null;
...
async function iniciarWhatsApp() {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (qr) {
      ...
      console.log("QR Code disponível no painel do bot.");
    }
    ...
    if (connection === "close") {
      ...
      if (shouldReconnect) {
        ...
        console.log("Sessão deslogada. Use o botão Desconectar WhatsApp no painel para gerar novo QR.");
      }
    }
  });
...
app.get("/api/qr", (_req, res) => {
  ...
  const qr = qrAtual || null;
  res.json({
    ok: true,
    estado: estadoAtual,
    conectado: estadoAtual === "conectado",
    qrDisponivel: Boolean(qr),
    qr,
    qrImageUrl: qr
      ? "https://api.qrserver.com/v1/create-qr-code/?size=280x280&margin=8&data=" + encodeURIComponent(qr)
      : null
  });
});
...
app.post("/api/whatsapp/desconectar", async (_req, res) => {
  try {
    desconexaoManualEmAndamento = true;
    estadoAtual = "desconectando";
    qrAtual = null;

    if (sock) {
      try { await sock.logout(); } catch (error) { console.log("Logout WhatsApp:", error.message); }
      try { sock.end?.(); } catch {}
      sock = null;
    }
    ...
    res.json({ ok: true, estado: estadoAtual, message: "WhatsApp desconectado. Um novo QR Code será gerado no painel." });
  } catch (error) {
    console.error("Erro ao desconectar WhatsApp:", error.message);
    estadoAtual = "erro_desconectar";
    res.status(500).json({ ok: false, error: error.message });
  } finally {
    setTimeout(() => { desconexaoManualEmAndamento = false; }, 5000);
  }
});
