...
app.get("/api/qr", async (_req, res) => {
  try {
    const qrDataUrl = qrAtual
      ? await QRCode.toDataURL(qrAtual, { width: 280, margin: 1 })
      : null;
    ...
    res.status(500).json({ ok: false, estado: estadoAtual, error: error.message });
  }
});
